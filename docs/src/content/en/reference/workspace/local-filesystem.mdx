...
<PropertiesTable
  ...
  },
  {
    ...
    type: 'string[]',
    description:
      'Additional paths that are allowed beyond basePath. Useful with `contained: true` to grant access to specific directories without disabling containment entirely. Relative paths are resolved against `basePath`. Absolute paths are used as-is.',
    isOptional: true,
    defaultValue: '[]',
  },
  {
