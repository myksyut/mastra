...
describe('Score Schemas', () => {
  ...
  describe('scoreRecordSchema', () => {
    it('accepts a complete score record', () => {
      ...
      expect(record.scorerName).toBe('relevance');
  ...
  describe('scoreInputSchema', () => {
    it('accepts valid user input', () => {
      ...
      expect(input.scorerName).toBe('relevance');
    });
  ...
  describe('scoresFilterSchema', () => {
    it('accepts all filter options', () => {
      const filter = scoresFilterSchema.parse({
        ...
        traceId: 'trace-1',
        spanId: 'span-1',
        scorerName: ['relevance', 'accuracy'],
        experimentId: 'exp-1',
        environment: 'production',
      });
      expect(filter.scorerName).toEqual(['relevance', 'accuracy']);
    });

    it('accepts single scorer name as string', () => {
      const filter = scoresFilterSchema.parse({ scorerName: 'relevance' });
      expect(filter.scorerName).toBe('relevance');
    });
